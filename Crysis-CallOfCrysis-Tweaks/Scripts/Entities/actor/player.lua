...
function Player:ResetHack()
		...
		System.SetCVar("g_suitCloakEnergyDrainAdjuster", 0.05);
...
function Player:GameplayFixClassicChanged(classic)
	if(classic == 0) then
		System.SetCVar("g_suitSprintMultiplier_speedMode_gotFullEnergy", 2);
		System.SetCVar("g_suitSprintMultiplier_speedMode_gotLowEnergy", 2);
		System.SetCVar("g_suitSprintMultiplier_speedMode_outOfEnergy", 2);
		System.SetCVar("g_suitSpeedJumpExtraForceInViewDirection_Horizontal", 1.75);
		System.SetCVar("g_suitArmorHealthValue", 200);
	else
		System.SetCVar("g_suitSprintMultiplier_speedMode_gotFullEnergy", 3);
		System.SetCVar("g_suitSprintMultiplier_speedMode_gotLowEnergy", 3);
		System.SetCVar("g_suitSprintMultiplier_speedMode_outOfEnergy", 3);
		System.SetCVar("g_suitSpeedJumpExtraForceInViewDirection_Horizontal", 2.5);
	...
	elseif (self ~= g_localActor or stats.thirdPerson or stats.isOnLadder) then
		...
		elseif ((stats.firstPersonBody or 0)>0 and not ghostPit) then
			...
			if (not item or not item.item:IsMounted() ) then
				self:HideAttachment(0, "lower_body", isZeroG, false);--isZeroG);
			end
		else
			self:DrawSlot(0,0);
		end
